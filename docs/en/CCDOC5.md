...
![](../images/concept-05-appendix.png)

In this appendix, we first clarify the meanings of `NAME` and `QSTR`, which were postponed in the main text. After that, we include `t0.def`, `t3.out.c`, and `comcom.h` as reference material.

First, here again are the definitions of `NAME` and `QSTR`.
...
In one sentence, their meanings are:
...
<table class="plain-table">
  <thead>
    <tr>
      <th>Meta-variable</th>
      <th>Meaning</th>
      <th>What it matches in PEN.def, for example</th>
    </tr>
  </thead>
  <tbody>
    <tr>
    ...
    <tr>
      <td>QSTR</td>
      <td>a string enclosed in double quotes</td>
      <td>" is not "</td>
    </tr>
  </tbody>
</table>

In `t0.def`, these two are used differently according to their roles.

In the current context, `NAME` stands for a meta-variable name that appears in `PEN.def`. Therefore it appears only on the syntax-rule side. If you look only at the generation rules of `NAME` itself, it seems to output the matched string unchanged.

For example, when `PEN.def` contains `THIS`, that becomes part of a function name on the `PEN.c` side.
...
Here, `THIS` is not output bare as `THIS`. It is transformed inside the higher-level rule into `_THIS(E)`.

So `NAME` is not normally used by itself. It is always used as part of some larger rule, and surrounding material such as `_` or `(E)` is added there.

The same kind of observation applies to `QSTR`, but `QSTR` can appear on both the syntax-rule side and the generation-rule side.

- On the syntax-rule side, `QSTR` becomes the comparison target passed to `check()`
- On the generation-rule side, `QSTR` is emitted as the argument to `print()`
...
What is slightly confusing here is the meaning of `print`.

In the direct execution example for `PEN.def`, the `print("a");` written in the generation rule ultimately writes `a` into the output file. That is exactly what we saw in the original `This is a pen` example.

But from the standpoint of `t0.def`, the situation is different. `t0.def` is a definition whose input is `PEN.def` and whose output is `PEN.c`. Therefore the input pattern

```c
print(" is not ");
```

appearing inside `PEN.def` is emitted by `t0.def` into `PEN.c` as the C code
...
<table class="plain-table">
  <thead>
    <tr>
      <th>Definition</th>
      <th>Rule being focused on</th>
      <th>Input processed by that rule</th>
      <th>Output at that stage</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>PEN.def</td>
      <td>generation rule print("a");</td>
      <td>This</td>
      <td>a</td>
    </tr>
    <tr>
...
Below are `t0.def`, `t3.out.c`, and `comcom.h` as reference material.
...
At this point we have followed the overall structure of a self-defined compiler-compiler from start to finish.

Most of the time, people use ready-made tools such as `yacc` or `bison`, and there is little reason to step into their internals, let alone into the stage where a system generates itself. This topic is not the shortest path to practical results. Its value lies elsewhere: it lets you follow a mechanism built almost entirely out of logic, all the way to the end.

The role of this document is to present that structure in a form that can be surveyed from start to finish. If you have read this far, then the document has already served its purpose.
